...
if __name__ == "__main__":
    ...
    r2d2_state = Robot_State(xpos=user_args.get("xpos", 0), ypos=user_args.get("ypos", 0), heading=user_args.get("heading", math.pi/4), store_data=config_args.get(
        "store_data"), is_sim=config_args.get("is_sim") if is_raspberrypi() else not is_raspberrypi())  # Let user define if on the pi, otherwise set to True
    ...
    mission_state = Mission_State(robot=r2d2, base_station_coord=(user_args.get("base_lat", 42.444250), user_args.get("base_long", -76.483682)),
                                  init_control_mode=ControlMode.LAWNMOWER if user_args.get("lawnmower", True) else ControlMode.ROOMBA)
    ...
    if config_args.get("is_transmit"):  # Set to true when the rpi/robot is communicating w/ the GUI
        packet_sender = threading.Thread(target=send_packet_to_gui, args=(
            1, r2d2_state, transmittor), daemon=True)  # Thread to read and send robot properties
        packet_sender.start()
