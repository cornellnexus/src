...
def avoid_obstacle_logic(robot_state):
    ...
    while True:
        ...
        # bug 2
            ...
            if (
                calculate_dist(curr_pos, last_pos_on_line)
                > robot_state.threshold_to_recalculate_on_line
            ):
                robot_state.phase = robot_state.prev_phase
                phase_change(robot_state)
                return None
...
def execute_boundary_following(robot_state):
    ...
    if lf_ultrasonic.distance() < side_margin_sensor:
        ...
        desired_heading = (
            curr_heading - math.pi if curr_heading > math.pi else curr_heading + math.pi
        )
        # error is arbitrary
        heading_err = 10
