...
class DataBase:
    def __init__(self, robot):
        ...
        self.core_data = {
            ...
            "position_pid": [
                ...
                robot.robot_state.position_kd,
            ],
            "position_noise": robot.robot_state.position_noise,
            "heading_pid": [
                robot.robot_state.heading_kp,
                robot.robot_state.heading_ki,
                robot.robot_state.heading_kd,
            ],
        }
    ...
    def __str__(self):
        return (
            "phase: "
            + str(self.core_data["phase"].value)
            + ",\n"
            + "state [x, y, heading]: ["
            + str(self.core_data["state"][0, 0])
            + ", "
            ...
            + ",\n"
            + "is_sim: "
            + str(self.core_data["is_sim"])
            + ",\n"
            + "plastic_level: "
            + str(self.core_data["plastic_level"])
            + ",\n"
            + "battery: "
            + str(self.core_data["battery"])
            + ",\n"
            + "move_dist: "
            + str(self.core_data["move_dist"])
            + ",\n"
            + "acceleration [x, y, z]: "
            + str(self.core_data["acceleration"])
            + ",\n"
            + "magnetic_field [x, y, z]: "
            + str(self.core_data["magnetic_field"])
            + ",\n"
            + "gyro_rotation [x, y, z]: "
            + str(self.core_data["gyro_rotation"])
            + ",\n"
            + "position_pid [proportional factor, integral factor, derivative factor]: "
            + str(self.core_data["position_pid"])
            + ",\n"
            + "position_noise: "
            + str(self.core_data["position_noise"])
            + ",\n"
    ...
    def update_data(self, name, x=None, y=None, z=None):
        ...
        if name in optional_keys:
            if x != None:
                self.core_data[name] = [
                    x,
                    self.core_data[name][1],
                    self.core_data[name][2],
                ]
            if y != None:
                self.core_data[name] = [
                    self.core_data[name][0],
                    y,
                    self.core_data[name][2],
                ]
            if z != None:
