...
class Graph:
    ...
    def __init__(self, long_min, long_max, lat_min, lat_max):
        ...
        def calc_step(long_min,long_max,lat_min,lat_max):
            long_range = geopy.distance.distance((long_min,lat_min),(long_max, lat_min)).meters
            lat_range = geopy.distance.distance((long_min,lat_min),(long_min,lat_max)).meters
        ...
        def generate_nodes(start_long,start_lat,row_num, col_num, long_step, lat_step):
            ...
            for i in range(col_num):
                for j in range(row_num):
                    if i % 2 == 0:
                        node = Node(origin[0] + i*long_step, origin[1] + j*lat_step)
                        traversal_path.append(node)
                        nodes_dict[node.get_coords()] = node
                    elif i % 2 == 1:
                        long_pos = origin[0] + i*long_step
                        lat_pos = origin[1] + ((row_num - 1) * lat_step) - j*lat_step
                        node = Node(long_pos, lat_pos)
                        traversal_path.append(node)
                        nodes_dict[node.get_coords()] = node
            return traversal_path, nodes_dict


        self.long_step, self.lat_step, self.col_num, self.row_num = calc_step(long_min,long_max,lat_min,lat_max)
        self.traversal_path, self.nodes_dict = generate_nodes(long_min,lat_min,self.row_num,self.col_num,self.long_step, self.lat_step) 
