...
class PathTraversal:
    def engine():
        ...
        TIME_STEP = (
            0.1  # is this the time by which we are going to sleep after each movement?
        )
        ...
        for curr_goal_ind in range(
            ...
            distance_away = math.hypot(
                float(predicted_state[0]) - curr_goal[0],
            ...
            while distance_away > ALLOWED_DIST_ERROR:
                ...
                (limited_cmd_v, limited_cmd_w) = limit_cmds(
                    cmd_v, cmd_w, MAX_V, ROBOT_RADIUS
                )
