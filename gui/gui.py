...
def setup_gui():
    ...
    left_col = [
        ...
        [sg.Image(key="-PROGRESS-", data=image_data[0])],
        [
            sg.Image(key="-MINIMAP-", data=image_data[1]),
            sg.Image(key="-CAMERA-", data=image_data[2]),
        ],
    ]
    right_col = [
        ...
        [
            sg.Button("Autonomous", key="-CONTROL_MODE_BUTTON-"),
            sg.Button("Track Location"),
            sg.Button("Traversal Phase"),
            sg.Button("Simulation"),
            sg.Button("Startup Base Station"),
            sg.Button("Read RPI Comms"),
        ],
        [
            sg.Multiline(
                str(robot_data),
    ...
    while True:  # Event Loop
        ...
        if event == "Simulation":
            simulation_thread = threading.Thread(
                target=run_simulation, args=(1,), daemon=True
            )
...
if not close_gui:
    input_data = (
        get_input_data()
    )  # Runs the gui popup asking for latitude and longitude bounds
    bounds = (
        input_data["long_min"],
        input_data["long_max"],
        input_data["lat_min"],
    ...
    if bounds != None:
        ...
        (
            ...
            make_robot_symbol()
        )  # Create a circle and wedge objet for robot location and heading, respectively
        ...
        if is_sim:
            ...
            robot_loc_file = open(
                CSV_PATH + "/datastore.csv", "r"
            )  # open csv file of robot location
        ...
        robot_data = RobotData(
            "phse:1;p_weight:00.0;acc:0.00,0.00,0.00;n_dist:00.0;rot:00.00;last_n:000.00,000.00;vel:0.00;next_n:000.00,000.00;coords:000.00,000.00,000.00;bat:000;ctrl:1"
        )
