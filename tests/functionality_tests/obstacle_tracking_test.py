...
class TestObstacleTracking(unittest.TestCase):
    def populate_standard_obstacle_data(self):
        mock_data_path = (
            ROOT_DIR + "/tests/functionality_tests/csv/ultrasonic_values.csv"
        )
        ...
        mock_data_file.write(
            "(100, 550, 600, 600, 600, 'turns towards one side of obstacle')\n"
        )
        ...
        mock_data_file.write("(350, 400, 400, 600, 600, '')\n")
        mock_data_file.write(
            "(600, 400, 400, 600, 600, 'corner of obstacle no longer in view')\n"
        )
        mock_data_file.write("(600, 400, 400, 600, 600, '')\n")
        mock_data_file.write(
            "(600, 600, 400, 600, 600, 'left sensor of robot hanging over obstacle')\n"
        )
        ...
        mock_data_file.write(
            "(600, 600, 450, 600, 600, 'left sensor decreases because passing by corner')\n"
        )
        ...
        mock_data_file.write("(600, 600, 600, 600, 600, '')\n")
        mock_data_file.write(
            "(600, 600, 600, 600, 600, 'goal reached, sensor value ends')\n"
        )
    ...
    def test_init(self):
        ...
        result_path = (
            ROOT_DIR + "/tests/functionality_tests/csv/avoid_obstacle_result.csv"
        )
        ...
        result_file = open(
            ROOT_DIR + "/tests/functionality_tests/csv/avoid_obstacle_result.csv", "r"
        )
        result_content = list(
            map(lambda string: string.rstrip("\n"), result_file.readlines())
        )
        result_file.close()
        expected = []
        for i in range(2):
    ...
    def test_min_value(self):
        ...
        self.assertEqual(
            ...
            min(
                r2d2.robot_state.max_sensor_range,
                ((r2d2.robot_state.width + r2d2.robot_state.width_margin) / 2)
                / math.cos(
                    math.radians((180 - r2d2.robot_state.sensor_measuring_angle) / 2)
                )
                + r2d2.robot_state.front_sensor_offset,
            ),
        )

    def test_detection_range(self):
        wide_robot_state = Robot_State(
            xpos=0,
            ypos=0,
            heading=math.pi / 2,
            epsilon=0.2,
        ...
        skinny_robot_state = Robot_State(
            xpos=0,
            ypos=0,
            heading=math.pi / 2,
            epsilon=0.2,
        ...
        self.assertEqual(
            self.difference_in_threshold(
                1,
                wide_robot.robot_state.max_sensor_range,
                wide_robot.robot_state.detect_obstacle_range,
            ),
            True,
        )
        self.assertEqual(
            self.difference_in_threshold(
                1, 165.1, skinny_robot.robot_state.detect_obstacle_range
            ),
            True,
        )
        self.assertEqual(
            self.difference_in_threshold(
                0, 165.1, skinny_robot.robot_state.detect_obstacle_range
            ),
            False,
        )
        self.assertEqual(
            self.difference_in_threshold(
                1, 200, skinny_robot.robot_state.detect_obstacle_range
            ),
            False,
        )
