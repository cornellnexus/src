...
def main():
    ...
    parser.add_argument(
        "-n", "--no-gui", action="store_true", help="suppress OpenCV gui"
    )

    parser.add_argument(
        "-d", "--debug-images", action="store_true", help="output debug detection image"
    )
    ...
    for filename in options.filenames:
        ...
        for i, detection in enumerate(detections):
            ...
            if options.camera_params is not None:
                ...
                if _HAVE_CV2:
                    _draw_pose(overlay, options.camera_params, options.tag_size, pose)
