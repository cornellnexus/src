...
def pulse_handler_right(pin):
	print("check2")
	if (counter_right==10):
		odometry(counter_right, r, L)     #Do odometry when right wheel gets 10 pulses
	else:     
		counter_right+=1
...
def odometry(counter_right, r, L):
	dtheta_right=(2*math.pi*counter_right)/ppr  #angular displacement of right wheel
	#dtheta_left=(2*math.pi*counter_left)/ppr    #angular displacement of left wheel
	...
	if (test):
		print("angular displacement is"+str(dtheta_right))		#+"\n"+"angular displacement is"+str(angular_dis)
	counter_right=0
    #counter_left=0
    
#test= BasicMotorController()
#test.setup()
